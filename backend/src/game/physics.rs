...
pub fn apply_snake_with_collisions(
  ...
  for _ in 0..step_count {
    ...
    let dot_value = clamp(dot(normalize(raw_head), normalize(corrected_head)), -1.0, 1.0);
...
fn resolve_mountain_contact(head: Point, mountain: &super::environment::MountainInstance) -> Option<(Point, Point)> {
