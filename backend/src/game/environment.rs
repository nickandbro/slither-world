...
pub const LAKE_MIN_DEPTH: f64 = BASE_PLANET_RADIUS * 0.10;
pub const LAKE_MAX_DEPTH: f64 = BASE_PLANET_RADIUS * 0.17;
...
pub const DESERT_CACTUS_COUNT: usize = 8;
pub const FOREST_TREE_COUNT: usize = TREE_INSTANCE_COUNT - DESERT_CACTUS_COUNT;
pub const DESERT_BIOME_ANGLE: f64 = 0.86;
pub const DESERT_CACTUS_MIN_ANGLE: f64 = 0.34;
...
const DESERT_BIOME_SEED: u32 = 0xf53c9e17;
...
pub struct TreeInstance {
    // Serialized under the legacy "trees" field for protocol compatibility.
    // width_scale >= 0.0 => forest tree; width_scale < 0.0 => desert cactus.
    pub normal: Point,
    pub width_scale: f64,
...
pub fn desert_biome_center() -> Point {
    let mut rng = SeededRng::new(DESERT_BIOME_SEED);
    random_on_sphere(&mut rng)
}
...
impl Environment {
    pub fn generate() -> Self {
        ...
        let rand_range =
            |rng: &mut SeededRng, min: f64, max: f64| min + (max - min) * rng.next_f64();
        ...
        let desert_center = desert_biome_center();
        let desert_min_dot = DESERT_BIOME_ANGLE.cos();
        let in_desert = |candidate: Point| dot(candidate, desert_center) >= desert_min_dot;
        let is_in_lake =
            |candidate: Point| sample_lakes(candidate, &lakes).boundary > LAKE_EXCLUSION_THRESHOLD;

        let mut trees = Vec::with_capacity(TREE_INSTANCE_COUNT);

        let tree_min_dot = TREE_MIN_ANGLE.cos();
        let mut tree_normals: Vec<Point> = Vec::with_capacity(FOREST_TREE_COUNT);
        let is_invalid_forest_tree =
            |candidate: Point| is_in_lake(candidate) || in_desert(candidate);
        for _ in 0..FOREST_TREE_COUNT {
            let candidate = pick_sparse_normal(
                &mut rng,
                tree_min_dot,
                &tree_normals,
                &is_invalid_forest_tree,
            );
        ...
        let cactus_min_dot = DESERT_CACTUS_MIN_ANGLE.cos();
        let mut cactus_normals: Vec<Point> = Vec::with_capacity(DESERT_CACTUS_COUNT);
        let is_invalid_cactus = |candidate: Point| is_in_lake(candidate) || !in_desert(candidate);
        for _ in 0..DESERT_CACTUS_COUNT {
            let candidate = pick_sparse_normal(
                &mut rng,
                cactus_min_dot,
                &cactus_normals,
            ...
            let height_scale = rand_range(&mut rng, min_height_scale, max_height_scale);
            let twist = rand_range(&mut rng, 0.0, PI * 2.0);
            cactus_normals.push(candidate);
            trees.push(TreeInstance {
                normal: candidate,
                // Negative width_scale tags this as a cactus while keeping the wire format unchanged.
        ...
        let is_invalid_mountain = |candidate: Point| is_in_lake(candidate) || in_desert(candidate);
...
fn create_lakes(seed: u32, count: usize) -> Vec<Lake> {
    ...
    for _ in 0..count {
        ...
        let center = pick_lake_center(radius, &lakes, &mut rng, desert_center, desert_min_dot);
...
fn pick_lake_center(
    ...
    for _ in 0..80 {
        ...
        if dot(candidate, desert_center) >= desert_min_dot {
            continue;
        }
        let mut ok = true;
        for lake in lakes {
    ...
    for _ in 0..MOUNTAIN_OUTLINE_SMOOTH_PASSES {
        for i in 0..MOUNTAIN_OUTLINE_SAMPLES {
            ...
            for offset in -radius..=radius {
                let idx =
                    (i as isize + offset).rem_euclid(MOUNTAIN_OUTLINE_SAMPLES as isize) as usize;
