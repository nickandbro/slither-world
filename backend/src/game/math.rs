...
pub fn collision_distance_for_angular_radii(angular_radius_a: f64, angular_radius_b: f64) -> f64 {
    let angular_distance = (angular_radius_a + angular_radius_b).max(0.0);
    2.0 * (angular_distance * 0.5).sin()
}

pub fn collision_with_angular_radii(
    a: Point,
    b: Point,
    angular_radius_a: f64,
    angular_radius_b: f64,
) -> bool {
    let threshold = collision_distance_for_angular_radii(angular_radius_a, angular_radius_b);
    if threshold <= 0.0 {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn base_collision_radius_matches_legacy_threshold() {
        let a = Point {
            x: 0.0,
            y: 0.0,
            z: 0.0,
        ...
        let inside = Point {
            x: COLLISION_DISTANCE * 0.99,
            y: 0.0,
            z: 0.0,
        };
        let outside = Point {
            x: COLLISION_DISTANCE * 1.01,
            y: 0.0,
            z: 0.0,
        };
    ...
    #[test]
    fn larger_radii_expand_collision_range_and_are_symmetric() {
        let a = Point {
            x: 0.0,
            y: 0.0,
            z: 0.0,
        ...
        assert!(!collision_with_angular_radii(a, b, base, base));
        assert!(collision_with_angular_radii(a, b, grown, grown));
        assert_eq!(
            collision_with_angular_radii(a, b, base, grown),
            collision_with_angular_radii(a, b, grown, base),
        );
    }
}
