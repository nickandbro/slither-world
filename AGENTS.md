...
- Pellet visuals use a slither-style multi-layer sprite stack in color buckets (`THREE.Points`) for high counts: dark under-shadow + bright core + near/far additive glow layers with seeded per-pellet orbital wobble; inner/far glow layers render without depth testing to avoid terrain clip artifacts while near-side horizon culling (with margin) prevents glow bleed from the far hemisphere. Keep updates allocation-light and preserve per-pellet terrain grounding so sprites stay on top of elevated/sunken terrain.
