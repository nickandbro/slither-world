...
- In dev/e2e, digestion diagnostics are available via `window.__SNAKE_DEBUG__.getSnakeIds()` and `window.__SNAKE_DEBUG__.getSnakeDigestionInfo(id)` (snapshot/visual counts, strengths, and max applied bulge).
...
- Multiplayer WebSocket payloads are custom binary frames (versioned header). Current protocol version is `7`; when the protocol changes, deploy frontend and backend together. The server still accepts JSON `Text` frames for backwards compatibility, but always sends binary. Client codec lives in `frontend/src/game/wsProtocol.ts`. State/init snapshots include `u16 total_players` plus a per-session view-scoped player list; the server always includes the local player and includes remote players only when they have a visible non-stub snake window for that session view. Player payload entries include `f32 girth_scale` (after `oxygen`), and pellet payload entries are encoded as `u32 pellet_id` + quantized normal (`i16 x/y/z`) + `u8 color_index` + `u8 size`.
...
- Small pellet digestion behavior is split intentionally: non-growing digestions are emitted for immediate bump/ring visuals (stacked per tick with a capped batch size), while score growth emits growing digestions that add tail length.
...
- Digestion bulges are applied in tube-ring space with identity-tracked progress, and the visual start is anchored one node behind the head (using head-segment length over current curve length) rather than a fixed body percentage. Bulge intensity scales down as snake girth increases so larger snakes show subtler swallow bumps.
