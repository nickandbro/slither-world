...
- Boosting is length-backed on the server. While boosting, snakes drain tail length smoothly over time and auto-stop at a per-life boost floor set from the spawned snake length (never below `MIN_SURVIVAL_LENGTH`); on spawn/respawn, score initializes to the spawned snake length. `PlayerSnapshot` includes `scoreFraction` for the radial score interval HUD, while the top-left score remains integer. The head-anchored radial gauge is shown only while boosting (with fade in/out) and uses whole-number center text.
