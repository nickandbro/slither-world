...
- Boosting is length-backed on the server. While boosting, snakes drain tail length smoothly over time (with auto-stop at the minimum survival floor), and `PlayerSnapshot` includes `scoreFraction` for the radial score interval HUD. The radial rim represents the current per-score interval and can be counterbalanced by eating while boosting.
