...
- When validating local steering response/body jitter mechanics specifically, use the focused suite: `cd frontend && npm run test:e2e -- e2e/steering-response.spec.ts --project=chromium --workers=1` (or `cd frontend && npm run test:e2e -- --grep @steering-response --project=chromium --workers=1`).
...
- Debug UI controls (collider/day-night/debug panel toggles) are enabled in `import.meta.env.DEV` or when `VITE_E2E_DEBUG=1`.
...
- Frontend renderer is WebGL-only.
- Legacy renderer settings are hard-deprecated: startup strips URL `renderer` query params and removes legacy localStorage keys (`spherical_snake_renderer`, `spherical_snake_webgpu_msaa_samples`) instead of migrating them.
- Optional adaptive quality (disabled by default as of February 10, 2026) can be toggled via localStorage:
  - `spherical_snake_adaptive_quality=1|0` (dynamic DPR only).
...
- In dev/e2e, `window.__SNAKE_DEBUG__.getRendererInfo()` reports `{ activeBackend, webglShaderHooksEnabled }` for renderer assertions.
...
- Small pellet digestion behavior is merged per player per tick: consumed pellet growth fractions are aggregated into one digestion event with weighted bump strength; each digestion has an intake delay + settle window, and when digestion reaches the tail its growth is applied authoritatively by draining tail backlog at a dynamic per-tick budget (`base + mult * sqrt(backlog)`, clamped) that is split across movement substeps so boost substepping does not create bursty tail growth.
- Tail node commits from digestion are paced at a maximum of one node per movement substep; excess fractional growth stays in `tail_extension` for later substeps (carryover). Growth-node placement continues the current tail arc (not `pos_queue` history), reseeds tail history queues for continuity, and preserves rapid multi-commit smoothness.
- Boost floor handling is digestion-aware: when boost reaches minimum length, pending digestion growth can be burned as fuel before hard-stopping boost so reserve feels responsive even while growth is still traveling down the body.
...
- `scripts/tail-growth-playtest.sh` — launches/attaches to local dev, enables tail-growth debug capture, and writes run artifacts under `output/tail-growth-tests/<run-label>/` (`report.json`, `tail-events.json`, `samples.json`, optional screenshot).
- `scripts/tail-growth-recorder.mjs` — browser-side collector used by the playtest script; captures rx/render tail-growth samples plus all-player human snapshots and emits incident classification (`rx_jump`, `client_tail_end_jump`, `all_player_snapshot_jump`, etc.) for tail-pop diagnosis.
...
- Snake skin UV progression includes fractional tail extension so stripe bands and color patterns advance smoothly as length accrues (no "only at full-node commit" pop). Snapshot interpolation for `snakeDetail='full'` blends length-units (`snakeLen + tailExtension`) and re-derives `{ snakeTotalLen, tailExtension }` across commit boundaries to avoid one-frame shrink/pops under rapid growth. Tail-tip rendering prefers authoritative replicated `tailTip` when available; otherwise the client eases tail-extension distance in world units (rate-limited grow/shrink with commit-boundary carry decay, smoothed tail base-length tracking, and per-player continuity state) and blends extension direction toward raw tail tangent near full extension to reduce overshoot/snap-back artifacts.
- Menu skin screens render a separate 3D preview snake as an overlay pass (separate scene/camera) after the main world/lake passes; it clears depth before rendering so it does not contaminate the multipass pellet occlusion pipeline (`renderer.clearDepth()`).
- Pointer aiming uses a 3D curved arrow overlay rendered after the main world/lake passes in `frontend/src/render/core/sceneRuntime/overlays/pointerArrow.ts`; it clears depth before rendering so the arrow appears above everything while still self-occluding correctly (`renderer.clearDepth()`).
...
- Digestion bulges are applied in tube-ring space with identity-tracked progress, and the visual start is anchored near the neck at a fixed fractional start (`DIGESTION_START_NODE_INDEX = 0.125`, i.e. `1/8` segment from node 0). Keep the near-head influence as a gradual buildup (not peak at the seam) so the neck does not appear overly thick while preserving smooth slither-style curvature.
...
- Lakes are rendered as per-lake spherical meshes masked with `onBeforeCompile`.
...
- Fishbowl crack visuals use WebGL shader hooks (`onBeforeCompile`).
- Renderer bootstrap lives in `frontend/src/render/webglScene.ts`; scene/runtime orchestration lives in `frontend/src/render/core/sceneRuntime/createSceneRuntimeImpl.ts` with subsystem modules under `frontend/src/render/core/sceneRuntime/{environment,snake,pellets,overlays,render,debug,utils}`. `frontend/src/render/core/sceneRuntime.ts` is kept as a compatibility re-export.
