...
- Snake body visuals are smooth-shaded and use a repeat-wrapped stripe mask texture (`map` + `emissiveMap`) to create ring-band stripes along the tube; generated tail caps include UVs so the stripe bands continue down the cap instead of snapping to a solid color.
