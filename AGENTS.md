...
- Multiplayer WebSocket payloads are custom binary frames (versioned header). Current protocol version is `14`; when the protocol changes, deploy frontend and backend together. Join frames include `FLAG_JOIN_DEFER_SPAWN` so clients can connect/update identity without immediate spawn (used by the pre-spawn menu flow) plus `FLAG_JOIN_SKIN` to attach an optional skin pattern (`u8 skin_len` then `skin_len * (u8 r,g,b)`; max 8). Client input is split: `TYPE_INPUT` is axis+boost only, and `TYPE_VIEW` carries `view_center/view_radius/camera_distance` (sent at lower rate to drive view-scoped replication). Client codec lives in `frontend/src/game/wsProtocol.ts`. State/init snapshots include `u16 total_players` plus a per-session view-scoped player list; the server always includes the local player and includes remote players only when they have a visible non-stub snake window for that session view. Player payload entries are compact: `u16 net_id` + packed flags + quantized scalars, and snake points are oct-encoded (`i16 ox, i16 oy` per node). Pellets are no longer sent inside every `TYPE_STATE`; they are replicated via `TYPE_PELLET_RESET` (full) plus `TYPE_PELLET_DELTA` (adds/updates/removes at lower rate). Pellet entries remain `u32 pellet_id` + quantized normal (`i16 x/y/z`) + `u8 color_index` + `u8 size`.
...
- Outbound backpressure: per-session `TYPE_STATE` is latest-state-wins (single-slot overwrite). If a client stalls, intermediate state frames are dropped and the client receives current state quickly once it resumes (prevents bufferbloat and unbounded server memory).
- Ordered outbound frames are bounded and split by priority: a bounded `outbound_hi` queue for `TYPE_INIT` / `TYPE_PLAYER_META` / `TYPE_PELLET_RESET`, and a bounded `outbound_lo` queue for `TYPE_PELLET_DELTA` (a slow client cannot grow server memory without bound).
